...
```
Since Node-RED can only be accessed from your tailnet devices, you could consider removing basic authentication from Node-RED dashboard.  That way no login screen will be displayed anymore, to improve the [WAF](https://en.wikipedia.org/wiki/Wife_acceptance_factor) factor of your home automation.  Because having to login over and over again can be rather annoying for the family.  Of courses for other use cases (for example in enterprise networks) it might be advised to keep on using the dashboard behind a login screen.
